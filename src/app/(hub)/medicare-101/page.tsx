...
function SystemConnections({ links, className }: { links: SystemLink[], className?: string }) {
  return (
    <div className={`${styles.systemConnections} ${className ?? ''}`}>
  ...
  return (
    <PageShell signal="neutral">
          ...
          {mistakes.map((m) => (
            ...
            >
              <div className={styles.mistakeHeader}>
                <div className={styles.mistakeNumWrap}>
                  <span className={styles.mistakeBigNum}>{m.num}</span>
                  <span className={styles.mistakeNumBadge}>Mistake</span>
                </div>
      ...
      >
        ...
        {/* Kill phrases */}
        <div className={styles.killSectionEntry}>
          <p className={styles.sectionLabel}>Quick Reference</p>
          <h3 className={`display-lg ${styles.killSectionTitle}`}>
            Phrases That Kill Sales
          </h3>
          <p className={`body-md ${styles.killSectionIntro}`}>
            Every phrase an agent should never say — and what to say instead. Each maps back to the pattern that creates it and the Certainty System pillar that fixes it.
          </p>
        </div>
          ...
          {killPhrases.map((row, i) => (
            <div key={i} className={styles.killCard}>
              <div className={styles.killRow}>
                <div className={`${styles.killCol} ${styles.killColKill}`}>
                  <p className={styles.killColLabelKill}>❌ What kills the call</p>
                  <p className={styles.killPhrase}>{row.kill}</p>
                </div>
                <div className={styles.killDivider} />
                <div className={`${styles.killCol} ${styles.killColClose}`}>
                  <p className={styles.killColLabelClose}>✅ What closes it</p>
                  {row.isNever
              ...
              <SystemConnections links={row.systemLinks} className={styles.inKillCard} />
            </div>
          ))}
        </div>
      </motion.section>
